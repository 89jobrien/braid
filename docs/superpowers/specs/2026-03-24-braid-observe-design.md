...
**Definition of done:** a completed run can be inspected from stored events.
...
`list()` sorts sessions by `written_at` from meta (falling back to directory mtime). `prune()` deletes the oldest directories beyond the keep threshold.
...
impl SessionStore {
    ...
    /// Print a session's event timeline to stdout.
    pub fn print(&self, id: &SessionId) -> Result<()>;
